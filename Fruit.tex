...
it can use blocks mined by $A$ to replace blocks mined by honest players.
			%all other players receive the adversarial block before the honest block
		\end{itemize}
		%If an adversary $A$ controls the delivery of messages of the network, it can, by delaying some messages, use a block mined by $A$ to replace a block mined by some honest player. Therefore erase the honest block.
		%If the adversary controls the delivery of messages, it can replace an honest player's block with its own block by ensuring that all honest players receive the adversarial block before the honest block.
	\end{block}
...
\end{frame}
\note{
当一个对手$A$掌握了整个网络的消息传递，\\
他可以通过延迟消息的传递，使得他能用自己挖出来的\\
恶意的区块代替诚实的玩家玩出来的诚实的区块。\\
从而相当于把诚实的玩家的区块删除掉了。
}
...
\begin{frame}
	...
	%\begin{block}{Validness of blocks and chains}
%		A block $b = (h_{-1}, \eta, m, h)$ is valid w.r.t. (a predecessor block) $b_{-1} = (h_{-1}', \eta', m', h')$ if
%		\begin{itemize}
%			\item $h_{-1} = h'$
%			\item ${\rm H.ver}\big((h_{-1}, \eta, m), h\big) = 1$
%			\item $h < D_p = p \cdot 2^{\kappa}$
%		\end{itemize}
%		A sequence of blocks $chain = (b_0, \dots, b_l)$ is valid if
%		\begin{itemize}
%			\item $h_{-1} = h'$
%			\item ${\rm H.ver}\big((h_{-1}, \eta, m), h\big) = 1$
%			\item $h < D_p = p \cdot 2^{\kappa}$
...
\begin{frame}
	...
	\begin{block}{Fruit mining}
		\begin{itemize}
			...
			\item 2-for-1 trick: the prefix and suffix of $H$'s output for block and fruit mining.% resp.%, respectively
		\end{itemize}
	\end{block}
	\vspace{-3pt}
	...
	\end{block}
	%\begin{itemize}
	%	\item Running an instance of Nakamoto’s blockchain protocol, but instead of directly storing the records m inside the blockchain, the records are put inside ``fruit'' $f$
	%	\item We require the fruits to ``hang'' from a block in the chain which is not too “far” from the block which records the fruit
	%	\item These fruits themselves requires solving some proof of work, with a \textit{different hardness parameter} $p_f$
	%	\item In each round, honest players simultaneously mine for a fruit and a block
	%	\item Whenever a player successfully mines a fruit it broadcasts it to all other players
	%\end{itemize}
\end{frame}
...
\begin{frame}
	...
	%\insblk{Preventing block withholding attack}{PO1}
	...
	\end{block}
	%\vspace{10pt}
	%\insblk{Preventing fruit withholding attack}{PO2}
